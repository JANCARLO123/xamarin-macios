			...
			using (var writer = new StreamWriter (makefile, false, new UTF8Encoding (false))) {
				...
				foreach (var target in allTargets) {
					...
					if (target is MacClassicTarget) {
						...
						string outputBitnessDir = target.IsBCLProject ? "" : "x86/";
						string testSuffix = target.IsBCLProject ? "Tests" : "";
						writer.WriteLine ("\t$(Q) {2}/bin/{3}Debug{1}/{0}.app/Contents/MacOS/{0}", make_escaped_name + testSuffix, target.Suffix, CreateRelativePath (Path.GetDirectoryName (target.ProjectPath), Path.GetDirectoryName (makefile)), outputBitnessDir);
						writer.WriteLine ();
